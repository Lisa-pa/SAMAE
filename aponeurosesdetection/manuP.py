...
def panoManu(manu_pts_dict, path_img = None):
    ...
    for fsc in fnmatch.filter(manu_pts_dict.keys(), 'fsc_*'):
        ...
        inters_inf, inters_sup, spl = MUFeaM.findIntersections(spl_inf = spline_inf, spl_sup = spline_sup, listSpl = spl, start = pt_inter[1]/2, insertion= pt_inter[1])
    ...
    if path_img is not None:
        ...
        I[int(pt_inter[0]), int(pt_inter[1]),:] = [255,255,0]
        I[int(pt_inter[0])-1, int(pt_inter[1]),:] = [255,255,0]
        I[int(pt_inter[0])+1, int(pt_inter[1]),:] = [255,255,0]
        I[int(pt_inter[0]), int(pt_inter[1])-1,:] = [255,255,0]
        I[int(pt_inter[0]), int(pt_inter[1])+1,:] = [255,255,0]
